...
public class NoOfHeadWithFlipUpdate {
    int[][] seg;

    public NoOfHeadWithFlipUpdate(int n) {
        seg = new int[4 * n][2];
    }
    ...
    public void update(int index, int low, int high, int left, int right) {
        // update prev remaining updates and propagate downwards
        if (seg[index][1] != 0) {
            seg[index][0] = (high - low + 1) - seg[index][0];
            // propagate
            if (low != high) {
        ...
        if (low >= left && high <= right) {
            seg[index][0] = (high - low + 1) - seg[index][0];
            if (low != high) {
                seg[2 * index + 1][1] = ~seg[2 * index + 1][1];
                seg[2 * index + 2][1] = ~seg[2 * index + 2][1];
            }
            return;
        }
        // partial overlap
    ...
    public static void main(String[] args) {
        ...
        NoOfHeadWithFlipUpdate segTree = new NoOfHeadWithFlipUpdate(n);
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (type == 1) {
                ...
                segTree.update(0, 0, n - 1, left, right);
            } else if (type == 2) {
